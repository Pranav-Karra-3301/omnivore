use crate::table_extractor::{TableData, TableExtractor};
use regex::Regex;
...
impl ContentExtractor {
    ...
    fn parse_course_block(&self, element: scraper::ElementRef) -> Option<CourseInfo> {
        ...
        let title_selector =
            Selector::parse(".course_codetitle, .courseblocktitle, .course-title").ok()?;
        ...
        if let Ok(desc_selector) =
            Selector::parse(".courseblockdesc, .course-description, .description")
        {
    ...
    fn extract_credits(&self, text: &str) -> Option<String> {
        let credit_pattern =
            regex::Regex::new(r"\((\d+(?:-\d+)?)\s*(?:credits?|cr\.?|units?)\)").ok()?;
    ...
    fn extract_lists(&self, document: &Html) -> Vec<ListContent> {
        ...
        if let Ok(selector) = Selector::parse("ul, ol") {
            for list in document.select(&selector) {
                ...
                if parent_html.contains("nav")
                    || parent_html.contains("menu")
                    || parent_html.contains("sidebar")
                    || parent_html.contains("breadcrumb")
                {
                    continue;
                }
                ...
                if items.len() > 2 && items.len() < 50 {
                    // Filter out huge navigation lists
                    let title = None;
